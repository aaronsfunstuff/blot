...
function smile() {
  const smileCenterX = 62.5;
  const smileCenterY = 40;
  const radius = 10;
  const startAngle = 0;
  ...
  for (let angle = startAngle; angle <= endAngle; angle += 0.1) {
    const x = smileCenterX + radius * Math.cos(angle);
    const y = smileCenterY - radius * Math.sin(angle);
    smilePoints.push([x, y]);
  }

  finalLines.push(smilePoints);
}
